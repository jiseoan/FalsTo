...
if ($isfirstlast) {
  echo '<image style="position:absolute; left:'.$x.'px; top:0px; cursor:pointer;" src="image/'.$manLangCode.'/movefirst.png" alt="처음" onclick="goPage(1);"/>';
  $x += ($imgwidth1 + $interval1);
  echo '<image style="position:absolute; left:'.$x.'px; top:0px; cursor:pointer;" src="image/'.$manLangCode.'/moveprev.png" alt="이전" onclick="goPage('.($curpage > 1 ? ($curpage-1) : 1).');"/>';
...
if ($isfirstlast) {
  echo '<image style="position:absolute; left:'.$x.'px; top:0px; cursor:pointer;" src="image/'.$manLangCode.'/movenext.png" alt="다음" onclick="goPage('.($curpage < $maxpgno ? ($curpage+1) : $maxpgno).');"/>';
  $x += ($imgwidth1 + $interval1);
  echo '<image style="position:absolute; left:'.$x.'px; top:0px; cursor:pointer;" src="image/'.$manLangCode.'/movelast.png" alt="맨끝" onclick="goPage('.$maxpgno.');"/>';
}
else if ($isprevnext) {
  echo '<image style="position:absolute; left:'.$x.'px; top:0px; cursor:pointer;" src="image/'.$manLangCode.'/movenext.png" alt="다음" onclick="goPage('.($curpage < $maxpgno ? ($curpage+1) : $maxpgno).');"/>';
}
